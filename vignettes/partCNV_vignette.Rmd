...
```
If you start with a SingleCellExperimetn object, you can also normalize and log transform your counts manually: Counts for each cell are divided by the total counts for the cell, and then multiplied by the scale.factor which is 10000 here. Counts then need to be transformed using function `log1p`. 
