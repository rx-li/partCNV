---
title: "Identify locally aneuploid cells from scRNA-seq using partCNV"
author:
- name: Ziyi Li
  affiliation: Department of Biostatistics, the University of Texas MD Anderson Cancer Center
  email: zli16@mdanderson.org
- name: Ruoxing Li
  affiliation: Department of Biostatistics, the University of Texas Health Science Center
package: partCNV
output:
...
vignette: |
  %\VignetteIndexEntry{partCNV_vignette}
...
```
If you start with a SingleCellExperimetn object, you can also normalize and log transform your counts manually: Counts for each cell are divided by the total counts for the cell, and then multiplied by the scale.factor which is 10000 here. Counts then need to be transfromed using `log1p` function.
